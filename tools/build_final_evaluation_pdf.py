...
def build() -> None:
    ...
    rows = [
        ...
        ["발표 체계성 10점", "비전, 문제 정의, WBS, 기술, 구현, 활용 흐름", "왜 만들었고 어떻게 만들었는지 순서대로 설명"],
        ["질의응답 5점", "ADR 4개, 앱 구조, 개발 환경, 빌드/배포 단계", "선택 이유를 기록 기준으로 답변"],
        ["개발자 기본소양 10점", "기술 설명, 아키텍처, 시행착오, 테스트, 코드 품질", "읽지 않고 이해한 내용으로 설명"],
        ["시연 데모 5점", "30초 데모 순서, 앱 화면, 핵심 기능, 임팩트 포인트", "사용자가 실제로 쓰는 장면처럼 보여주기"],
    ]
    table(c, 56, 420, [130, 330, 280], rows, 58)
    rounded(c, 56, 76, 728, 92, 20, colors.HexColor("#E9FFFB"), colors.HexColor("#B8EFE8"), 1)
    draw_wrapped(
        c,
        "발표 전략: 점수표의 모든 질문을 슬라이드 안에 숨기지 않고, 제목과 문장으로 직접 드러낸다. "
        "그래서 발표자는 '비전 - 문제 - 해결 - 구현 - 검증 - 활용' 순서만 따라가도 평가 항목을 자연스럽게 충족한다.",
        84,
        130,
        674,
        font=FONT_BOLD,
    ...
    # 7. WBS progress
    header(c, "WBS 진행 현황: MVP 핵심 경험 완료, 저장과 리포트 확장 중", "WBS", page)
    wbs = [
        ...
        ("14주차", "기록과 알림 확장", 80, "날짜별 저장, 월간 달력, 알림 현황, DB 확장 준비"),
        ("15주차", "테스트와 최종 발표", 90, "PDF, 데모, README, 배포 가이드 정리"),
    ]
    y = 390
    ...
    rows = [
        ...
        ["5", "기록/알림", "정원식", "진행", "날짜별 저장, 월간 보기, 마감 알림 현황"],
        ["6", "검증/발표", "정원식", "진행", "typecheck, 데모 시나리오, 최종 평가 PDF"],
    ]
